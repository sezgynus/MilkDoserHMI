...
public class UsbService extends Service {
    ...
    public void onCreate() {
        ...
        dir_tmr = new Timer();
        dir_tmr.schedule(dir_tmr_task,0,1);
    }

    final TimerTask dir_tmr_task = new TimerTask() {
        @Override
        public void run() {
            if(timer_run)count++;
            if(count>2)
    ...
    private class ReadThread extends Thread {
        ...
        public void run() {
            while(keep.get()){
                ...
                int value = serialInputStream.read(buff,0, buff.length);
                if(value != -1) {
                    String str = toASCII(value);
                    mHandler.obtainMessage(SYNC_READ, buff).sendToTarget();
                }
            }
        }
